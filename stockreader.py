...
class StockReader():
  def __init__(self, ticker, start=None, end=None, exchange="WIKI"):
    ticker = ticker.upper()
    self.symbol = ticker
    if(type(start) == tuple):
    ...
    print('{} Stocker Initialized. Data covers {} to {}.'.format(
      self.symbol,
      self.min_date,
      self.max_date
    ))
  ...
  def evaluate_prediction(self, start_date=None, end_date=None, nshares = None):
    ...
    train = self.stock[(self.stock['Date'] < start_date) & (self.stock['Date'] > (start_date - pd.DateOffset(years=self.training_years)))]
    ...
    if not nshares:
      ...
      plt.title('{} Model Evaluation from {} to {}.'.format(
        self.symbol,
        start_date,
        end_date
      ))
  ...
  def changepoint_date_analysis(self, search=''):
    self.reset_plot()
